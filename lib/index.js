...
async function run(ctx, resumeSessionId, io) {
  ...
  // Resolve the current model's context window lazily (for /context bricks).
  (async () => {
    try {
      ...
      if (llm?.listModels && sel?.provider && sel?.model) {
        ...
        if (cur?.contextWindow) store.set({ ctxWindow: Number(cur.contextWindow) });
      }
    } catch { /* non-fatal: /context falls back to 128k */ }
  })();
  ...
  const store = createStore(
    {
      ...
      extPanel: null, // { id, title, lines, scroll, selected } while a brick panel is open
      theme: themeName(),
      ctxWindow: null, // resolved lazily from llm.listModels (for /context)
