...
import { a2aSend, a2aProbe, A2A_AGENTS } from "./channel/a2a.js";
...
const COMMANDS = [
  ...
  "/compact", "/jobs", "/plan", "/goal", "/search", "/trajectory", "/feedback", "/tab", "/agents",
];
...
const BUSY_SAFE_COMMANDS = [
  "/jobs", "/config", "/mode", "/model", "/help", "/sessions", "/goal", "/plugins", "/new", "/search",
];
...
async function run(ctx, resumeSessionId, io) {
  ...
  // Brick-extension seam: companion plugins (dsh-tui-compact/usage/...) inject
  // "tuiExtensions" and register commands/panels/status fields/themes/hooks.
  ...
  const store = createStore(
    {
      ...
      searchPanel: null, // { hits, selected, query } while /search results shown
      trajPanel: null, // { events, selected, detail } while /trajectory open
      pluginsPanel: null, // { plugins, selected, detail } while /plugins open
      extPanel: null, // { id, title, lines, scroll } while a brick panel is open
      theme: themeName(),
  ...
  const ctl = {
    ...
    onSuggestionNav: (delta) => {
      ...
      if (suggestions?.length) {
        ...
        else if (store.get().trajPanel) ctl.trajNav(delta);
        else if (store.get().searchPanel) ctl.searchNav(delta);
    ...
    async openExtPanel(id) {
      ...
      store.set({ extPanel: { id, title: def.title, lines: ["加载中…"], scroll: 0 } });
      input?.setMenu("ext");
      try {
        const res = await def.load(store.get());
        const lines = Array.isArray(res?.lines) ? res.lines : [String(res ?? "")];
        store.set({ extPanel: { id, title: def.title, lines, scroll: 0 } });
      } catch (err) {
        store.set({ extPanel: { id, title: def.title, lines: [`加载失败: ${err.message}`], scroll: 0 } });
      }
    },
    closeExtPanel() {
    ...
    extNav(delta) {
      ...
      if (!p) return;
      const max = Math.max(0, (p.lines?.length || 0) - 24);
      store.set({ extPanel: { ...p, scroll: Math.max(0, Math.min(max, (p.scroll || 0) + delta)) } });
    },
    leaderKey(key) {
    ...
    // session full-text search (/search <query>).
    async doSearch(query) {
      const sq = ctx.get("sessionQuery");
      if (!sq?.searchSessions) {
        notice("error", "session search unavailable");
        return;
      }
      try {
        const page = await sq.searchSessions({ query, limit: 10 });
        input?.setMenu("search");
        store.set({ searchPanel: { hits: page.hits ?? [], selected: 0, query } });
      } catch (error) {
        notice("error", `搜索失败: ${error.message}`);
      }
    },
    closeSearch() {
      input?.setMenu(null);
      store.set({ searchPanel: null });
    },
    searchNav(delta) {
      const menu = store.get().searchPanel;
      if (!menu || !menu.hits?.length) return;
      const n = menu.hits.length;
      const next = (((menu.selected ?? 0) + delta) % n + n) % n;
      store.set({ searchPanel: { ...menu, selected: next } });
    },
    searchPick() {
      const menu = store.get().searchPanel;
      if (!menu || !menu.hits?.length) return;
      const hit = menu.hits[menu.selected ?? 0];
      notice("info", `继续该会话: dsh --profile tui --resume ${hit.id}`);
      ctl.closeSearch();
    },
    // trajectory step-through (/trajectory). Steps = aggregated transcript
    // entries (messages/tool cards), not raw deltas.
    openTrajectory() {
      const transcript = buildTranscript(store.get().events);
      input?.setMenu("traj");
      store.set({ trajPanel: { events: transcript, selected: 0, detail: "" } });
    },
    closeTrajectory() {
      input?.setMenu(null);
      store.set({ trajPanel: null });
    },
    trajNav(delta) {
      const menu = store.get().trajPanel;
      if (!menu || !menu.events?.length) return;
      const n = menu.events.length;
      const next = (((menu.selected ?? 0) + delta) % n + n) % n;
      store.set({ trajPanel: { ...menu, selected: next, detail: "" } });
    },
    trajDetail() {
      const menu = store.get().trajPanel;
      if (!menu || !menu.events?.length) return;
      const ev = menu.events[menu.selected ?? 0];
      const text =
        ev.text ?? ev.reasoning ?? ev.preview ?? ev.args ?? ev.name ?? JSON.stringify(ev);
      store.set({
        trajPanel: { ...menu, detail: menu.detail === "" ? text : "" },
      });
    },
    // message feedback (/feedback up|down [note]).
    async feedback(verdict, note) {
      const last = store.get().lastAssistantId;
      if (!last) {
        notice("warning", "还没有可反馈的助手消息");
        return;
      }
      const { writeFileSync, readFileSync, mkdirSync } = await import("node:fs");
      const { join } = await import("node:path");
      const path = join(CONFIG_PATH, "..", "feedback.json");
      let all = [];
      try {
      ...
      all.push({ messageId: last, verdict, note: note ?? "", ts: Date.now() });
      try {
        mkdirSync(join(path, ".."), { recursive: true });
        writeFileSync(path, JSON.stringify(all, null, 2));
        notice("info", `已记录反馈（${verdict === "up" ? "👍" : "👎"}）`);
      } catch (error) {
        notice("error", `反馈写入失败: ${error.message}`);
      }
    },
    // A2A dispatch: @hermes/@claude/@codex/@dsh <task> -> local agent network.
    async a2aSend(name, text) {
      notice("info", `A2A 派活 @${name}: ${text.slice(0, 60)}`);
      try {
        const r = await a2aSend(name, text);
        store.set({
          events: [
            ...store.get().events,
            { kind: "a2a", name, text, result: r.text, ok: r.state === "TASK_STATE_COMPLETED", ms: r.ms },
          ],
        });
      } catch (error) {
        store.set({
          events: [
            ...store.get().events,
            { kind: "a2a", name, text, result: `失败: ${error.message}`, ok: false, ms: 0 },
          ],
        });
      }
    },
    async a2aAgents() {
      ...
      for (const name of Object.keys(A2A_AGENTS)) {
        ...
        rows.push(`  ${probe.ok ? paint("✓", "success") : paint("✗", "error")} @${name}  ${probe.detail}`);
      }
      notice("info", `A2A 端点:\n${rows.join("\n")}`);
    },
    // Multi-session tabs: switch = hot-resume another session (single live
    ...
    // Show the live goal state (/goal) — reads the goals service snapshot.
    async showGoal() {
      ...
      try {
        ...
        if (!g) {
          notice("info", "当前无活跃目标 — 可让 agent 使用 goal 工具设定目标");
          return;
        }
    ...
    // plugins market (/plugins): aggregated bundles, builtin vs user-loaded.
    openPlugins() {
      const loader = ctx.get("loader");
      if (!loader?.entries) {
  ...
  input = new Input(io.stdin, {
    ...
    onSubmit: (text) => {
      ...
      if (trimmed.startsWith("/")) {
        ...
        if (command === "/feedback") {
          const rest = trimmed.slice("/feedback".length).trim();
          const m = rest.match(/^(up|down|👍|👎|good|bad)(?:\s+([\s\S]*))?$/i);
          if (!m) {
            notice("warning", "用法: /feedback up|down [备注]");
          } else {
            const verdict = ["up", "👍", "good"].includes(m[1].toLowerCase()) ? "up" : "down";
            ctl.feedback(verdict, m[2] ?? "");
          }
          return;
        }
        if (command === "/agents") {
        ...
        if (command === "/tab") {
          ...
          if (arg === "new") {
            ...
            notice("warning", "用法: /tab new | /tab <编号>");
          }
          return;
        }
        if (command === "/new") {
      ...
      // @agent prefix dispatches to the local A2A network.
      const a2aMatch = trimmed.match(/^@([a-zA-Z-]+)\s+([\s\S]+)$/);
      if (a2aMatch && A2A_AGENTS[a2aMatch[1]]) {
        ctl.a2aSend(a2aMatch[1], a2aMatch[2]);
        return;
      }
      ctl.submit(text);
    },
    ...
    onMenuNav: (delta) => {
      if (store.get().pluginsPanel) ctl.pluginsNav(delta);
      else if (store.get().trajPanel) ctl.trajNav(delta);
      else if (store.get().searchPanel) ctl.searchNav(delta);
      else if (store.get().jobsPanel) ctl.jobsNav(delta);
      else if (store.get().modeMenu) ctl.modeNav(delta);
    ...
    onMenuToggle: () => {
      if (store.get().pluginsPanel) ctl.pluginsDetail();
      else if (store.get().trajPanel) ctl.trajDetail();
      else if (store.get().searchPanel) ctl.searchPick();
    ...
    onMenuClose: () => {
      ...
      else if (store.get().trajPanel) ctl.closeTrajectory();
      else if (store.get().searchPanel) ctl.closeSearch();
...
function handleCommand(ctx, command, full, store, io) {
  ...
  switch (command) {
    ...
    case "/plan":
      notice("info", "plan mode: ask the agent to plan; base handles approval");
      break;
    default:
