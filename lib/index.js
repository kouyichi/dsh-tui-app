...
async function run(ctx, resumeSessionId, io) {
  ...
  const ctl = {
    ...
    onSuggestionNav: (delta) => {
      ...
      if (suggestions?.length) {
        ...
        if (store.get().trajPanel) ctl.trajNav(delta);
    ...
    // plugins market (/plugins): aggregated loaded plugin bundles.
    openPlugins() {
      const loader = ctx.get("loader");
      if (!loader?.entries) {
      ...
      try {
        for (const e of loader.entries()) {
          ...
          const rec = map.get(name) ?? { name, count: 0, disabled: 0, ids: [] };
          rec.count += 1;
      ...
      const plugins = [...map.values()].sort((a, b) => a.name.localeCompare(b.name));
      input?.setMenu("plugins");
      store.set({ pluginsPanel: { plugins, selected: 0, detail: "" } });
    },
    closePlugins() {
      input?.setMenu(null);
    ...
    pluginsNav(delta) {
      ...
      if (!menu || !menu.plugins?.length) return;
      const n = menu.plugins.length;
      const next = (((menu.selected ?? 0) + delta) % n + n) % n;
    ...
    pluginsDetail() {
      const menu = store.get().pluginsPanel;
      if (!menu || !menu.plugins?.length) return;
      const p = menu.plugins[menu.selected ?? 0];
      store.set({
        pluginsPanel: {
          ...menu,
  ...
  input = new Input(io.stdin, {
    ...
    onMenuExtra: () => {
      if (store.get().jobsPanel) ctl.jobsKill();
      else if (store.get().modelMenu) ctl.modelToggleEffort();
    },
    onMenuClose: () => {
      if (store.get().pluginsPanel) ctl.closePlugins();
