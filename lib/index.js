...
import { rankSlashItems } from "./slash-fuzzy.js";
...
/** Command descriptions for the slash menu (Hermes-style fuzzy scoring). */
const COMMAND_META = {
  "/help": "显示本帮助",
  "/quit": "退出 TUI（/exit /q）",
  "/new": "开启新会话",
  "/config": "状态栏显示开关",
  "/mode": "agent 模式：标准/code/minimal/cordis",
  "/model": "选择模型与推理力度（跨 provider）",
  "/plugins": "插件市场：浏览/装卸",
  "/resume": "恢复历史会话（CLI: --resume <id>）",
  "/sessions": "列出已持久化会话",
  "/jobs": "后台任务面板",
  "/goal": "目标状态面板",
  "/tab": "多会话标签：/tab new | /tab <编号>",
};
 ...
 * cordis.patch.yml — takes effect on next launch).
 * `rowId` is the loader row whose config receives the value:
 *   contextWindow → token-meter (its config schema accepts it)
 *   others        → compaction-basic (thresholdRatio/retainTokens/auto)
 * `null` = leave the dsh default untouched.
 */
const CONFIG_PARAMS = [
  { key: "contextWindow", label: "上下文窗口 (tokens)", min: 1000, max: 2000000, rowId: "token-meter", patchKey: "contextWindow" },
  { key: "compressThreshold", label: "压缩触发阈值 (0.05–1)", min: 0.05, max: 1, rowId: "compaction-basic", patchKey: "thresholdRatio" },
  { key: "compactRetainTokens", label: "压缩后保留 (tokens)", min: 100, max: 200000, rowId: "compaction-basic", patchKey: "retainTokens" },
  { key: "autoCompact", label: "自动压缩 (true/false)", bool: true, rowId: "compaction-basic", patchKey: "auto" },
];
...
/**
 * Apply the /config session parameters:
 *  - contextWindow → settings service (`llm-deepseek.defaultContextWindow`,
 *    sparse-patch, comment-preserving) — takes effect immediately.
 *  - compressThreshold / compactRetainTokens / autoCompact → the tui profile's
 *    cordis.patch.yml `compaction-basic` row (last-write-wins by id) — takes
 *    effect on the next dsh launch (engine config is construction-time).
...
function updateSessionParams(config, ctx) {
  // 1) contextWindow → settings service (runtime, immediate)
  if (config.contextWindow != null) {
    const settings = ctx?.get?.("settings");
    if (settings && typeof settings.update === "function") {
      try {
        settings.update("llm-deepseek", { defaultContextWindow: config.contextWindow });
      } catch (err) {
        /* best-effort */
      }
    }
  }
  // 2) compaction params → profile patch (restart-effective)
  try {
    const byRow = new Map(); // rowId -> [[patchKey, value], ...]
    for (const p of CONFIG_PARAMS) {
      if (p.rowId !== "compaction-basic") continue;
      if (config[p.key] == null) continue;
      if (!byRow.has(p.rowId)) byRow.set(p.rowId, []);
      byRow.get(p.rowId).push([p.patchKey, config[p.key]]);
    }
    let patch = "";
    try { patch = readFileSync(TUI_PATCH_PATH, "utf8"); } catch { /* new file */ }
    const lines = patch.length > 0 ? patch.split("\n") : [];
    // All compaction params restored to default: remove the whole block the
    // TUI wrote earlier so the stale values can't linger past a restart.
    const removeBlock = (rowId) => {
      let start = -1;
      for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes(`id: ${rowId}`)) { start = i; break; }
      }
      if (start < 0) return;
      let k = start - 1;
      ...
      for (let j = start + 1; j < lines.length; j++) {
        if (lines[j].trimStart().startsWith("- id:") && !lines[j].includes(rowId)) { end = j; break; }
      }
      lines.splice(start, end - start);
    };
    if (byRow.size === 0) {
      removeBlock("compaction-basic");
      writeFileSync(TUI_PATCH_PATH, lines.join("\n") + "\n");
      return; // all defaults — nothing to write
    }
    // Insert/replace one row block per target loader row.
    for (const [rowId, pairs] of byRow) {
      // Locate the existing row and its config block.
      let start = -1;
      for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes(`id: ${rowId}`)) { start = i; break; }
      }
      ...
      if (start >= 0) {
        for (let j = start + 1; j < lines.length; j++) {
          if (lines[j].trimStart().startsWith("- id:") && !lines[j].includes(rowId)) {
            end = j;
            break;
          }
        }
      ...
      const block = [
        ...
        `- id: ${rowId}`,
        "  config:",
        ...pairs.map(([k, v]) => `    ${k}: ${typeof v === "boolean" ? (v ? "true" : "false") : v}`),
      ];
...
async function run(ctx, resumeSessionId, io) {
  ...
  // 可变模型选择快照：installModelSelection 的钩子每次请求读它（assemble 时
  // current 落入 assembled）；updateSelection 直接改字段 → 当前 agent 下一个
  // 请求即用新模型（busy 时即"当前对话完成后自动起效"）。新 agent 的钩子读
  // 同一对象，同样生效。
  ...
  (async () => {
    ...
    if (loader?.entries) {
      try {
        // Count like the /plugins panel (grouped by package name) so the two
        // numbers agree instead of splash showing raw row count.
        const byName = new Map();
        for (const e of loader.entries()) {
          const name = e?.options?.name ?? e?.options?.id ?? "?";
          byName.set(name, (byName.get(name) ?? 0) + 1);
        }
        meta.plugins = byName.size;
      } catch {
        meta.plugins = 0;
      }
    }
  ...
  // Throttled: goal/changed can fire rapidly during a goal round.
  let lastGoalNoticeAt = 0;
  let goalUnsub = agent.ctx.on("goal/changed", (_s, { change }) => {
    const g = change?.goal ?? change;
    if (!g) return;
    const now = Date.now();
    if (now - (lastGoalNoticeAt ?? 0) < 2000) return;
    lastGoalNoticeAt = now;
  ...
  const ctl = {
    async submit(text) {
      ...
      try {
        ...
        await agent.whenIdle();
        // Session may have been disposed mid-turn (plugin reload via /plugins t,
        // interrupted teardown, tab switch race): flushing a dead session throws
        // "session ... is not live in this store" and would crash the whole TUI.
        try {
          await sessions.flush(agent.session);
        } catch {
          /* session gone — nothing to flush */
        }
      } finally {
        submitting = false;
        input?.setBusy(false);
    ...
    configEditSubmit(buffer) {
      ...
      if (text === "") {
        ...
        if (clamped !== value) {
          notice("info", `${param.label}: 已自动限制到 ${param.min}–${param.max} 范围（${value} → ${clamped}）`);
        }
        value = clamped;
      }
      const config = { ...store.get().config, [param.key]: value };
      saveConfig(config);
      updateSessionParams(config, ctx);
      ...
      notice(
        "info",
        param.rowId === "compaction-basic"
          ? `已设置 ${param.label} = ${value == null ? "默认（dsh 内置）" : value}。重启 dsh 后生效（已写入 tui profile 的 cordis.patch.yml）`
          : `已设置 ${param.label} = ${value == null ? "默认（dsh 内置）" : value}。已写入 settings.yaml，立即生效`
      );
    },
    configEditCancel() {
      const menu = store.get().configMenu;
    ...
    async openModel() {
      // Prefer the brick model catalog (all providers, verified models +
      // reasoning efforts); fall back to the current provider's list.
      let items = [];
      let effortOptions = null;
      try {
        const groups = await ext.modelCatalog();
        if (groups.length > 0) {
          ...
          for (const g of groups) {
            for (const m of g.items ?? []) {
              items.push({
                ...
                name: m.name ?? m.id,
                provider: g.provider,
                providerName: g.providerName ?? g.provider,
                contextWindow: m.contextWindow ?? m.context?.contextWindow,
                efforts: g.efforts ?? m.efforts ?? null,
              });
            }
          }
          // effort options for the current model (fall back to first group's)
          const cur = items.find((i) => i.provider === sel.provider && i.id === sel.model);
          effortOptions = cur?.efforts ?? groups[0]?.efforts ?? null;
        }
      } catch (error) {
        notice("error", `model catalog failed: ${error.message}`);
        return;
      }
      if (items.length === 0) {
        // legacy path: current provider only
        ...
        for (const m of list) {
          ...
          items.push({ id: m.id, name: m.name ?? m.id, provider: selection.provider, providerName: selection.provider, contextWindow: cw, efforts: null });
        }
        effortOptions = ["off", "low", "high", "max"];
      }
      if (items.length === 0) {
        notice("info", "没有可用的模型（检查 provider 配置: /providers）");
        return;
      }
      const sel = defaultModel.currentSelection();
      const selIdx = Math.max(0, items.findIndex((m) => m.provider === sel.provider && m.id === sel.model));
      input?.setMenu("model");
      store.set({
        modelMenu: {
          items,
          selected: selIdx,
          effort: sel.reasoningEffort ?? (effortOptions?.includes("high") ? "high" : (effortOptions?.[0] ?? "max")),
          efforts: effortOptions ?? items[selIdx]?.efforts ?? ["off", "low", "high", "max"],
        },
      });
    },
    closeModel() {
    ...
    modelNav(delta) {
      ...
      // switching rows also switches the effort list to that model's options
      store.set({
        modelMenu: {
          ...menu,
          selected: next,
          efforts: item?.efforts ?? menu.efforts,
          effort: (item?.efforts ?? menu.efforts ?? []).includes(menu.effort) ? menu.effort : ((item?.efforts ?? menu.efforts ?? [])[0] ?? menu.effort),
        },
      });
    },
    modelToggleEffort() {
      const menu = store.get().modelMenu;
      if (!menu) return;
      const opts = menu.efforts?.length ? menu.efforts : ["off", "low", "high", "max"];
      const idx = opts.indexOf(menu.effort);
    ...
    async modelSelect() {
      ...
      if (!menu || !menu.items?.length) return;
      const item = menu.items[menu.selected ?? 0];
      const effort = menu.effort ?? undefined;
      try {
        ...
        // apply immediately to the running agent (selection is a mutable
        // snapshot; installModelSelection reads it per request)
        ctl.updateSelection({ provider: item.provider, model: item.id, ...(effort ? { reasoningEffort: effort } : {}) });
        // 上边栏跟随：状态栏/splash 的 model 显示目标模型
        store.set({ meta: { ...store.get().meta, model: item.id, provider: item.provider } });
        const when = submitting ? "当前对话完成后自动起效" : "立即生效";
        notice("success", `已切换 ${item.providerName ?? item.provider}/${item.id}（推理力度 ${effort ?? "默认"}）— ${when}`);
      } catch (error) {
        notice("error", `切换失败: ${error.message}`);
      }
      ctl.closeModel();
    },
    /** Mutate the running agent's selection snapshot (immediate effect). */
    updateSelection(patch) {
      try {
        Object.assign(selection, patch);
      } catch (error) {
        notice("error", `updateSelection failed: ${error.message}`);
      }
    },
    ...
    async switchTab(i) {
      ...
      try {
        try { await sessions.flush(agent.session); } catch { /* session gone */ }
        channel.detach();
    ...
    async newTab() {
      ...
      try {
        ...
        refreshStats();
        return true; // opened — /btw relies on this to avoid sending to the main thread
      } catch (error) {
  ...
  const handleSubmit = async (text) => {
    ...
    if (trimmed.startsWith("/")) {
      ...
      // (use the store busy flag — `submitting` alone misses the window
      // between followup() and the turn-start event)
      if ((submitting || store.get().input?.busy) && DEFER_UNTIL_IDLE.has(command)) {
      ...
      if (command === "/mode") {
        ...
        if (arg !== "") {
          notice("warning", `未知模式参数: ${arg}（/mode 打开面板选择，无需参数）`);
          return;
        }
        ctl.openMode();
        return;
      ...
      if (command === "/goal") {
        // TUI's own goal command: /goal <objective> creates & arms a goal;
        // subcommands (edit/pause/clear/…) fall through to the kernel bridge.
        const arg = trimmed.slice("/goal".length).trim();
        if (arg === "" || !/^(edit|pause|clear|resume)\b/.test(arg)) {
          ctl.goalCmd(arg === "" ? undefined : arg);
          return;
        }
      }
    ...
    if (submitting || store.get().input?.busy) {
      notice("warning", "agent 正忙（Ctrl+C 可中断；Alt+Enter 可排队发送）");
      return;
    }
  ...
  input = new Input(io.stdin, {
    onChange: ({ buffer, cursor, vim }) => {
      ...
      input._suggestions = suggestions; // for Enter-to-accept in input.js
      store.set({
    ...
    onSuggestClose: () => {
      store.set({ input: { ...store.get().input, suggestions: [], selected: 0 } });
    },
    ...
    onQuestionSubmit: (text) => {
      ...
      if (opts.length > 0) {
        ...
        if (nums.length > 0) {
          ...
          if (digits && opts.length > 0 && parseInt(digits[0], 10) > opts.length) {
            // typed an out-of-range number: surface it instead of silently
            // submitting a weird custom answer (restore the question overlay)
            store.set({ question: q });
  ...
  input.suggestionsFor = (buffer) => {
    if (buffer.startsWith("/")) {
      // Hermes-style slash menu: rank by description-aware fuzzy score.
      ...
      const items = [...names].map((c) => ({
        text: c,
        description: COMMAND_META[c] ?? ext.commands.get(c)?.description ?? "",
      }));
      const ranked = rankSlashItems(items, buffer, (it) => ({ id: it.text, label: it.text, description: it.description }));
      // an exact full-command match hides the menu (the pick is done)
      return ranked[0]?.text === buffer ? [] : ranked;
    }
    if (buffer.startsWith("@")) {
      ...
      return [...new Set([...names, ...files])].slice(0, 12).map((text) => ({ text }));
    }
    return [];
  };
  input.acceptSuggestion = (sugg) => {
    ...
    let text = buffer;
    const value = typeof sugg === "string" ? sugg : sugg?.text ?? "";
    if (value.startsWith("/")) {
      text = value;
  ...
  switch (command) {
    ...
    case "/sessions": {
      ...
      persistence
        .list()
        .then((headers) => {
          if (headers.length === 0) notice("info", "暂无会话记录");
          else
            notice(
              "info",
