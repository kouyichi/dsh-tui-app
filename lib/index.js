...
const COMMANDS = [
  "/help", "/quit", "/config", "/model", "/resume", "/sessions", "/compact", "/jobs", "/plan", "/goal",
];
...
async function run(ctx, resumeSessionId, io) {
  ...
  const ctl = {
    ...
    openConfig() {
      input?.setConfigOpen(true);
      store.set({ configMenu: { selected: 0 } });
    },
    closeConfig() {
      input?.setConfigOpen(false);
  ...
  input = new Input(io.stdin, {
    ...
    onConfigNav: (delta) => ctl.configNav(delta),
    onConfigToggle: () => ctl.configToggle(),
    onConfigClose: () => ctl.closeConfig(),
  });
