...
const BUSY_SAFE_COMMANDS = ["/jobs", "/config", "/mode", "/model", "/help", "/sessions"];
...
async function run(ctx, resumeSessionId, io) {
  ...
  const store = createStore(
    {
      ...
      anim: { tps: 0, tick: 0 }, // whale spout animation state
  ...
  // Whale spout animation: tick every 400ms; spout height follows the token
  // rate (tps) sampled from the stats projection.
  const animTimer = setInterval(() => {
    const s = store.get();
    if (!s.anim) return;
    store.set({ anim: { tps: s.stats?.tps ?? 0, tick: s.anim.tick + 1 } });
  }, 400);
  ...
  const ctl = {
    ...
    async switchTab(i) {
      ...
      try {
        ...
        channel = createChannel(agent, (ev) => {
          store.set({ events: [...store.get().events, ev] });
          if (ev.kind === "assistant" && ev.id) store.set({ lastAssistantId: ev.id });
          refreshStats();
        });
  ...
  input = new Input(io.stdin, {
    ...
    onMenuToggle: () => {
      if (store.get().trajPanel) ctl.trajDetail();
    ...
    onMenuClose: () => {
      if (store.get().trajPanel) ctl.closeTrajectory();
