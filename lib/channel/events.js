...
export function previewResult(content) {
  ...
  try {
    if (typeof raw === "string" && raw.startsWith("[")) {
      // Very common shape: [{"type":"text","text":"..."}] — unwrap to the
      // inner text so cards show output instead of raw JSON.
      const arr = JSON.parse(raw);
      text = Array.isArray(arr)
        ? arr.map((b) => (typeof b === "string" ? b : b?.text ?? "")).join("").trim() || raw
        : raw;
    } else {
  ...
  function onEvent(event) {
    switch (event.type) {
      ...
      case "tool/result": {
        ...
        // drop the mapping once consumed so long sessions don't grow the map
        if (callId !== undefined) callNames.delete(callId);
      ...
      case "turn/end": {
        ...
        if (reason?.kind === "error") {
          ...
          emit({
            kind: "notice",
            tone: "warning",
            text: "输出达到 token 上限（max-tokens）",
          });
        } else if (reason?.kind === "aborted") {
          emit({ kind: "notice", tone: "warning", text: "回合已中断" });
        }
        break;
      }
      default:
