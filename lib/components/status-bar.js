...
export default function StatusBar({ status, meta, stats, config, ctl, pendingCount }) {
  const bits = [];
  if ((pendingCount ?? 0) > 0) bits.push(paint(`⏳${pendingCount}`, "warning"));
  if (meta?.model) bits.push(paint(meta.model, "accent"));
  if (meta?.mode) bits.push(paint(meta.mode, "accent"));
  if (config?.turns && stats) {
    bits.push(paint(`t${stats.turns}`, "dim"));
  }
  if (config?.steps && stats) {
    bits.push(paint(`s${stats.steps}`, "dim"));
  }
  ...
  // Brick-extension status fields (plain text, sorted by order).
  if (ctl?.extStatusFields) {
    for (const f of ctl.extStatusFields()) bits.push(f);
  }
