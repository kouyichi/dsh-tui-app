    ...
    transcript.map((entry) => {
        ...
        return h(
          ...
          showThinking
            ? h(
                ...
                fold === "expanded" ? h(Text, { dimColor: true, wrap: "wrap" }, `│ ${thinking.slice(0, 2000)}${thinking.length > 2000 ? "…" : ""}`) : null
              )
            : null,
          text !== ""
            ? h(
                Box,
                { key: "body", flexDirection: "column", marginTop: showThinking ? 1 : 0 },
                ...renderMarkdown(text)
              )
            : !showThinking
      ...
      if (entry.type === "a2a") {
        ...
        const body =
          fold === "hidden"
            ? null
            : h(Text, { key: "body", wrap: "wrap", dimColor: true }, `  ${entry.result.slice(0, 1500)}`);
        return h(
          Box,
          { key: entry.key, flexDirection: "column", marginLeft: 1 },
          h(Text, null, head),
          h(Text, { wrap: "wrap" }, `  ${entry.text.slice(0, 200)}`),
          body
        );
      }
      return null;
