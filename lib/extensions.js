...
import { registerTheme as registerPaletteTheme, setTheme as applyPaletteTheme, themeName as paletteThemeName } from "./theme/palette.js";

const paletteApi = () => ({
  registerTheme: registerPaletteTheme,
  setTheme: applyPaletteTheme,
...
export function createExtensions() {
  ...
  return {
    ...
    registerTheme(def) {
      ...
      try {
        const { registerTheme: registerPalette, setTheme: applyTheme, themeName } = paletteApi();
        registerPalette(def.name, def.codes);
        if (themeName() === def.name) applyTheme(def.name);
    ...
    addInputHook(hook) {
      ...
      return () => {
        ...
        // Leader-key bindings must be cleaned up too, or a unloaded brick's
        // ctrl+x shortcuts keep firing (ghost handlers on a dead plugin).
        if (hook.onLeader && typeof hook.onLeader === "object") {
          for (const k of Object.keys(hook.onLeader)) inputHooks.onLeader.delete(k);
        }
      };
    },
  };
