...
import TrajectoryPanel from "../components/trajectory-panel.js";
import PluginsPanel from "../components/plugins-panel.js";
import ExtPanel from "../components/ext-panel.js";
...
export default function App({ store, ctl }) {
  const {
    ...
    configMenu, modeMenu, modeCurrent, modelMenu, question, jobsPanel, searchPanel, trajPanel,
    pluginsPanel, extPanel, tabs, activeTab, fold,
  } = store.get();

  return h(
    Box,
    { flexDirection: "column", height: "100%" },
    h(TabBar, { tabs, activeTab }),
    h(
      Box,
    ...
    h(StatusBar, { status, meta, stats, config, ctl }),
    ...
    h(SearchPanel, { menu: searchPanel }),
    h(TrajectoryPanel, { menu: trajPanel }),
    h(PluginsPanel, { menu: pluginsPanel }),
    h(ExtPanel, { panel: extPanel }),
