...
export default function App({ store, ctl }) {
  const {
    ...
    configMenu, modeMenu, modeCurrent, modelMenu, question, jobsPanel,
    pluginsPanel, extPanel, tabs, activeTab, fold, pendingCount,
  } = store.get();
  ...
  return h(
    ...
    h(StatusBar, { status, meta, stats, config, ctl, pendingCount }),
