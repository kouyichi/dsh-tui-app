...
export class Input {
  ...
  constructor(stdin, opts = {}) {
    ...
    this.busy = false;
    this.vim = false;
    this.menu = null; // 'config' | 'mode' | 'model' while a menu is open
  ...
  handleEscape(seq) {
    ...
    if (seq.startsWith("\u001b[5~")) { this.opts.onTabSwitch?.(-1); return 3; } // PgUp
    if (seq.startsWith("\u001b[6~")) { this.opts.onTabSwitch?.(1); return 3; } // PgDn
