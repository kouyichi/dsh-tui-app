...
export class Input {
  ...
  onData(chunk) {
    ...
    while (i < chunk.length) {
      if (this.configOpen) {
        // /config menu mode: space toggles, arrows navigate, enter/esc close.
        const char = chunk[i];
        if (char === " ") {
          this.opts.onConfigToggle?.();
        } else if (char === "\u001b") {
          const consumed = this.handleEscape(chunk.slice(i));
          i += consumed;
          continue;
        } else if (char === "\r" || char === "\n") {
          this.opts.onConfigClose?.();
        } else if (char === "\u007f" || char === "\b") {
          this.opts.onConfigClose?.();
        }
        i += 1;
  ...
  handleEscape(seq) {
    ...
    if (seq.startsWith("\u001b[A")) {
      const nav = this.configOpen ? this.opts.onConfigNav : this.opts.onSuggestionNav;
      nav?.(-1);
      return 3;
    }
    if (seq.startsWith("\u001b[B")) {
