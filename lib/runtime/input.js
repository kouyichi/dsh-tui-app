...
export class Input {
  ...
  submit() {
    ...
    if (line.trim() === "") return;
    // Enter with an open slash suggestion menu accepts the selected command
    // (Hermes-style: type /, arrows, Enter to pick).
    if (line.startsWith("/") && Array.isArray(this._suggestions) && this._suggestions.length > 0) {
  ...
  handleEscape(seq) {
    if (seq.length < 2) {
      ...
      // Esc closes the open slash suggestion menu first (restores typing).
      if (this.buffer.startsWith("/") && Array.isArray(this._suggestions) && this._suggestions.length > 0) {
        this._suggestions = [];
        this.opts.onSuggestClose?.();
        return 1;
      }
